---
title: "Configura le stampanti per macOS"
description: "Scopri come configurare le stampanti IPP sui dispositivi macOS rilevando le stampanti disponibili sulla rete, identificando i loro indirizzi IP, generando un file mobileconfig e importando la configurazione per una perfetta integrazione della stampa."
---

Questo articolo spiega come configurare le stampanti IPP sui dispositivi macOS. Il processo prevede il rilevamento delle stampanti disponibili sulla rete, l'identificazione dei loro indirizzi IP e l'importazione della configurazione tramite un'impostazione MDM personalizzata.

## Scansiona le stampanti IPP in rete

*Se conosci già la configurazione della tua stampante, puoi saltare questa parte.*

1. Apri **Terminale** su un dispositivo macOS.
2. **Esegui** il comando seguente per rilevare tutte le stampanti disponibili tramite Internet Printing Protocol (IPP):

```bash
ippfind
```

3. **Nota** l'elenco delle stampanti rilevate e i relativi URI IPP.

## Identificare gli indirizzi IP della stampante

*Se conosci già la configurazione della tua stampante, puoi saltare questa parte.*

1. Per ciascuna stampante rilevata, esegui un comando ping per verificarne la disponibilità e ottenere il suo indirizzo IP:

```bash
ping [printer-hostname]
```

Sostituisci `[printer-hostname]` con il nome host effettivo visualizzato da ippfind

2. Registrare gli indirizzi IP e i corrispondenti URI IPP.

## Genera il file mobileconfig

Se hai bisogno di aiuto per generare il file mobileconfig con il comando ippfind e ping, puoi eseguire questo script sul tuo Mac mentre sei connesso alla rete dell'ufficio. Visualizzerà il file mobileconfig nel tuo terminale.
...
1. Accedi a **MDM > Profili.** Seleziona il profilo che desideri scegliere come target.
2. Seleziona **Aggiungi un'impostazione MDM personalizzata.**
3. Carica il seguente file **.mobileconfig**, sostituendo i segnaposto (IP, UUID, ecc.) con i dettagli della configurazione della stampante:
...
4. **Salva** e **distribuisci** il profilo.
