...
description: "IAM (Identity and Access Management) centralizes user management and access control, addressing challenges like uncontrolled access and regulatory compliance while offering features such as automated onboarding, custom SSO (Single Sign-On), and license management to optimize your SaaS environment."
---

IAM (Identity and Access Management) is a comprehensive solution that centralizes the management of your users and their access to applications.
...
<Note>
1. **Automated onboarding/offboarding** – Automatic account creation and deletion for your employees
2. **Custom SSO (Single Sign-On)** – Unified authentication for your applications
...
SSO (Single Sign-On) is an authentication method allowing a user to access multiple applications with a single credential (e.g., logging into Notion with Google).

**Benefits:** simplified password management, improved user experience, stronger security.

### SCIM (System for Cross-domain Identity Management)

SCIM (System for Cross-domain Identity Management) is a standardized protocol that automates the full user lifecycle (creation, updates, deletion) in applications via the identity provider.

### JIT provisioning (Just-in-Time)

JIT (Just-in-Time) provisioning automatically creates a user account upon first login via SSO, without prior manual configuration.
