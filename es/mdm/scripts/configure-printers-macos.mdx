---
title: "Configurar impresoras para macOS"
description: "Aprenda a configurar impresoras IPP en dispositivos macOS descubriendo impresoras disponibles en la red, identificando sus direcciones IP, generando un archivo mobileconfig e importando la configuración para una integración de impresión perfecta."
---

Este artículo explica cómo configurar impresoras IPP en dispositivos macOS. El proceso implica descubrir las impresoras disponibles en la red, identificar sus direcciones IP e importar la configuración a través de una configuración MDM personalizada.

## Escanear impresoras IPP en la red

*Si ya conoce la configuración de su impresora, puede omitir esta parte.*

1. Abra **Terminal** en un dispositivo macOS.
2. **Ejecute** el siguiente comando para descubrir todas las impresoras disponibles a través del Protocolo de impresión de Internet (IPP):

```bash
ippfind
```

3. **Tenga en cuenta** la lista de impresoras descubiertas y sus URI de IPP.

## Identificar las direcciones IP de la impresora

*Si ya conoce la configuración de su impresora, puede omitir esta parte.*

1. Para cada impresora descubierta, ejecute un comando ping para verificar su disponibilidad y obtener su dirección IP:

```bash
ping [printer-hostname]
```

Reemplace `[printer-hostname]` con el nombre de host real mostrado por ippfind

2. Registre las direcciones IP y los URI de IPP correspondientes.

## Generar el archivo mobileconfig

Si necesita ayuda para generar el archivo mobileconfig con el comando ippfind y ping, puede ejecutar este script en su Mac mientras está conectado a la red de su oficina. Mostrará el archivo mobileconfig en su terminal.
...
1. Acceda a **MDM > Perfiles.** Seleccione el perfil al que desea dirigirse.
2. Seleccione **Agregar una configuración MDM personalizada.**
3. Cargue el siguiente archivo **.mobileconfig**, reemplazando los marcadores de posición (IP, UUID, etc.) con los detalles de configuración de su impresora:
...
4. **Guardar** e **implementar** el perfil.
