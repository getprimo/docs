---
title: "Configurer les imprimantes pour macOS"
description: "Découvrez comment configurer les imprimantes IPP sur les appareils macOS en découvrant les imprimantes disponibles sur le réseau, en identifiant leurs adresses IP, en générant un fichier mobileconfig et en important la configuration pour une intégration transparente de l'impression."
---

Cet article explique comment configurer les imprimantes IPP sur les appareils macOS. Le processus consiste à découvrir les imprimantes disponibles sur le réseau, à identifier leurs adresses IP et à importer la configuration via un paramètre MDM personnalisé.

## Scanner les imprimantes IPP sur le réseau

*Si vous connaissez déjà la configuration de votre imprimante, vous pouvez ignorer cette partie.*

1. Ouvrez **Terminal** sur un appareil macOS.
2. **Exécutez** la commande suivante pour découvrir toutes les imprimantes disponibles via Internet Printing Protocol (IPP) :

```bash
ippfind
```

3. **Notez** la liste des imprimantes découvertes et leurs URI IPP.

## Identifiez les adresses IP de l'imprimante

*Si vous connaissez déjà la configuration de votre imprimante, vous pouvez ignorer cette partie.*

1. Pour chaque imprimante découverte, exécutez une commande ping pour vérifier sa disponibilité et obtenir son adresse IP :

```bash
ping [printer-hostname]
```

Remplacez `[printer-hostname]` par le nom d'hôte réel affiché par ippfind

2. Enregistrez les adresses IP et les URI IPP correspondants.

## Générer le fichier mobileconfig

Si vous avez besoin d'aide pour générer le fichier mobileconfig avec la commande ippfind et ping, vous pouvez exécuter ce script sur votre Mac pendant que vous êtes connecté au réseau de votre bureau. Il affichera le fichier mobileconfig dans votre terminal.
...
1. Accédez à **MDM > Profils.** Sélectionnez le profil que vous souhaitez cibler.
2. Sélectionnez **Ajouter un paramètre MDM personnalisé.**
3. Téléchargez le fichier **.mobileconfig** suivant, en remplaçant les espaces réservés (IP, UUID, etc.) par les détails de configuration de votre imprimante :
...
4. **Enregistrez** et **déployez** le profil.
