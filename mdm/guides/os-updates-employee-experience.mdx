---
title: "OS updates: Employee experience"
description: "Learn how administrators can monitor device update status and how employees receive OS update notifications on macOS and Windows."
---

## Monitoring device update status

Administrators can monitor the OS update status for all managed devices directly in the Primo cockpit. Each device displays one of three update statuses:

| Status             | Description                                                                                  |
| ------------------ | -------------------------------------------------------------------------------------------- |
| **Up to date**     | The device is running an OS version that meets your configured update deadline requirements. |
| **Not up to date** | The device is running an outdated OS version and the configured deadline has passed.         |
| **Not setup**      | No OS update policy has been configured for the device.                                      |

<Note>
  The update status is calculated based on your configured OS update deadline policy. Devices with MDM disabled will not display an update status.
</Note>

You can filter the device list by update status to quickly identify devices that require attention.

## Employee experience

### macOS 14 and above

![Screenshot: macOS 14 and above](/images/notion-import/employee-experience-for-os-updates__img-064__block-0005__1b243d9f.png)
...
![Screenshot: Before macOS 14](/images/notion-import/employee-experience-for-os-updates__img-065__block-0012__1b243d9f.png)
...
<Note>
  For all scenarios detailed above, the deadline applies from the "update availability date", not the configuration date of the policy. If you have employees on an older version for longer than the chosen deadline, they will be prompted to update their OS immediately.
</Note>
