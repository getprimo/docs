...
| Status | Meaning |
|---|---|
| **Enrolled** | The device is fully enrolled and the MDM agent is active. |
| **Partially enrolled** | Enrollment started but was not completed. |
| **Missing agent** | The FleetDM agent is no longer communicating with the server. |
| **Missing profile** | The MDM profile is absent or inactive on the device (macOS only). |
| **Enrolled in another MDM** | The device is currently managed by a different MDM. |
| **Pending** | Enrollment invitation sent; waiting for the employee to start installation. |
| **Offline** | The device has not communicated with the MDM server recently. |
...
<Tip>
Save time by automating employee re-enrolment. In **Settings > MDM**, enable **Auto-Unenrollment** and select the option to **automatically resend MDM invitations** to employees whose devices have been purged (i.e., devices that have been offline for a certain duration).
</Tip>
